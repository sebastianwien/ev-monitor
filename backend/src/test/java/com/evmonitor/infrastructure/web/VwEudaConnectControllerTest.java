...
class VwEudaConnectControllerTest {

    @Mock VwEudaConnectService service;
    @Mock User user;
    ...
    void setUp() {
        ...
        mvc = MockMvcBuilders.standaloneSetup(new VwEudaConnectController(service))
    ...
    void success_returnsConnectionStatus() throws Exception {
        when(service.connect(userId, carId, "skoda", "a@b.de", "pw")).thenReturn(
                new VwEudaConnectService.ConnectionStatus(carId, "skoda", "a@b.de", "VIN", "ACTIVE", null, null, null));
    ...
    void invalidCredentials_is422_not401() throws Exception {
        failWith(new VwEudaAuthException.InvalidCredentials());
        mvc.perform(post("/api/eu-data-act/cars/{carId}/connect", carId).contentType(MediaType.APPLICATION_JSON).content(BODY))
    ...
    void interactionRequired_is409() throws Exception {
        failWith(new VwEudaAuthException.InteractionRequired("termsAndConditions"));
        mvc.perform(post("/api/eu-data-act/cars/{carId}/connect", carId).contentType(MediaType.APPLICATION_JSON).content(BODY))
    ...
    void portalUnavailable_is503() throws Exception {
        failWith(new VwEudaAuthException.PortalUnavailable("down"));
        mvc.perform(post("/api/eu-data-act/cars/{carId}/connect", carId).contentType(MediaType.APPLICATION_JSON).content(BODY))
    ...
    void notEntitled_is403() throws Exception {
        failWith(new VwEudaConnectService.NotEntitledException());
        mvc.perform(post("/api/eu-data-act/cars/{carId}/connect", carId).contentType(MediaType.APPLICATION_JSON).content(BODY))
    ...
    void rateLimited_is429() throws Exception {
        failWith(new VwEudaConnectService.RateLimitedException());
        mvc.perform(post("/api/eu-data-act/cars/{carId}/connect", carId).contentType(MediaType.APPLICATION_JSON).content(BODY))
    ...
    void connectorRejection_isPassedThrough() throws Exception {
        failWith(new VwEudaConnectService.ConnectorRejectedException(429, "CAPACITY_REACHED", "voll"));
        mvc.perform(post("/api/eu-data-act/cars/{carId}/connect", carId).contentType(MediaType.APPLICATION_JSON).content(BODY))
                .andExpect(status().isTooManyRequests())
