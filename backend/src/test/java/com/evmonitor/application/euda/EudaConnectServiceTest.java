...
class EudaConnectServiceTest {

    private static final Map<String, String> COOKIES = Map.of("s_abc", "SESSION", "d_abc", "DEVICE");
    private static final EudaAutoSyncEntitlementService.Entitlement ENTITLED =
            new EudaAutoSyncEntitlementService.Entitlement(true, false, null);
    private static final EudaAutoSyncEntitlementService.Entitlement NOT_ENTITLED =
            new EudaAutoSyncEntitlementService.Entitlement(false, false, null);

    @Mock CarRepository carRepository;
    @Mock UserRepository userRepository;
    @Mock EudaAutoSyncEntitlementService entitlement;
    @Mock RateLimitService rateLimit;
    @Mock EudaLoginClientFactory clients;
    @Mock EudaLoginClient client;
    @Mock RestTemplate rest;
    @Mock User user;
    @Mock Car car;

    private EudaConnectService service;
    private final UUID userId = UUID.randomUUID();
    private final UUID carId = UUID.randomUUID();

    @BeforeEach
    void setUp() {
        service = new EudaConnectService(carRepository, userRepository, entitlement, rateLimit, clients, rest,
        ...
        lenient().when(rateLimit.tryConsumeEudaLogin(any())).thenReturn(true);
        lenient().when(clients.create(any())).thenReturn(client);
        lenient().when(client.login(any(), any())).thenReturn(COOKIES);
    }

    private EudaConnectService.ConnectionStatus status() {
        return new EudaConnectService.ConnectionStatus(carId, "skoda", "max@example.com", "TMBTESTVIN0000001",
                "ACTIVE", null, null, null);
    }

    @Test
    void connect_logsIn_andHandsOnlySsoCookiesToConnector() {
        when(rest.exchange(any(String.class), eq(HttpMethod.PUT), any(), eq(EudaConnectService.ConnectionStatus.class)))
                .thenReturn(ResponseEntity.ok(status()));

        EudaConnectService.ConnectionStatus result = service.connect(userId, carId, "skoda", " Max@Example.com ", "geheim");

        verify(clients).create("skoda");
        verify(client).login("max@example.com", "geheim");

        @SuppressWarnings("unchecked")
        ArgumentCaptor<HttpEntity<EudaConnectService.SessionHandover>> entity = ArgumentCaptor.forClass(HttpEntity.class);
        verify(rest).exchange(eq("http://connectors:8081/api/internal/euda/cars/" + carId + "/session"),
                eq(HttpMethod.PUT), entity.capture(), eq(EudaConnectService.ConnectionStatus.class));
        EudaConnectService.SessionHandover body = entity.getValue().getBody();
        assertNotNull(body);
    ...
    void connect_rejectsNotEntitled_withoutLogin() {
        ...
        assertThrows(EudaConnectService.NotEntitledException.class,
    ...
    void connect_rateLimited_withoutLogin() {
        when(rateLimit.tryConsumeEudaLogin(userId.toString())).thenReturn(false);

        assertThrows(EudaConnectService.RateLimitedException.class,
                () -> service.connect(userId, carId, "skoda", "a@b.de", "pw"));
        verifyNoInteractions(clients, rest);
    }
    ...
    void connect_whenLoginFails_doesNotCallConnector() {
        when(client.login(any(), any())).thenThrow(new EudaAuthException.InvalidCredentials());

        assertThrows(EudaAuthException.InvalidCredentials.class,
    ...
    void connect_passesConnectorRejectionThrough() {
        ...
        EudaConnectService.ConnectorRejectedException ex = assertThrows(EudaConnectService.ConnectorRejectedException.class,
                () -> service.connect(userId, carId, "skoda", "a@b.de", "pw"));
    ...
    void connect_whenConnectorUnreachable_reportsPortalUnavailable() {
        ...
        assertThrows(EudaAuthException.PortalUnavailable.class,
                () -> service.connect(userId, carId, "skoda", "a@b.de", "pw"));
    }
}
