 ...
 */
class VwEudaHistoricalExportTest {
    ...
    void takesSessionsFromRecords_skipsSessionsWithoutEnergy() throws Exception {
        List<VwEudaSession> sessions = parser.parse(export(historicalExport())).sessions();
    ...
    void acSession_mapsAllFieldsFromRecord() throws Exception {
        VwEudaSession s = parser.parse(export(historicalExport())).sessions().get(1);
    ...
    void dcSession_isMapped() throws Exception {
        VwEudaSession s = parser.parse(export(historicalExport())).sessions().get(0);
    ...
    void tooManyDistinctFieldsIsRejected_notBuffered() {
        ...
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i <= VwEudaJsonParser.MAX_DISTINCT_FIELDS; i++) {
