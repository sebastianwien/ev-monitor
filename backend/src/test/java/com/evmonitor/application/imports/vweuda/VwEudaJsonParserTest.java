...
class VwEudaJsonParserTest {

    private VwEudaJsonParser parser;

    @BeforeEach
    void setUp() {
        parser = new VwEudaJsonParser(new ObjectMapper());
    }
    ...
    void parsesVin() throws Exception {
        VwEudaParseResult result = parser.parse(sampleJson());
    ...
    void session1_isAcWithCorrectSocAndDuration() throws Exception {
        VwEudaSession s = parser.parse(sampleJson()).sessions().get(0);
    ...
    void session2_isDcFastCharge() throws Exception {
        VwEudaSession s = parser.parse(sampleJson()).sessions().get(1);
    ...
    void session3_isDcWithCorrectSoc() throws Exception {
        VwEudaSession s = parser.parse(sampleJson()).sessions().get(2);
    ...
    void sessionsHaveOdometerKm() throws Exception {
        List<VwEudaSession> sessions = parser.parse(sampleJson()).sessions();
    ...
    void telemetryOnlyDrop_lenient_returnsNoSessions() throws Exception {
        ...
        VwEudaParseResult result = parser.parse(this::telemetryOnlyJson, true);
    ...
    void brokenJson_lenient_stillThrows() {
        ...
        // VwEudaUnreadableException (siehe VwEudaImportServiceTest).
