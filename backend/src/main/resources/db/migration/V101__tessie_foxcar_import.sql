-- =============================================================================
-- Tessie Foxcar Fleet Import (Flyway Migration)
-- Processes tessie_raw_imports -> ev_log + ev_trip for 17 Foxcar vehicles.
-- Idempotent: ON CONFLICT DO NOTHING on inserts, processed flag prevents re-runs.
-- Runs automatically on deploy; inserts 0 rows if tessie_raw_imports is empty (local dev).
-- After running: POST /api/admin/soh/redetect to detect SoH for imported cars.
-- =============================================================================
...
CREATE TEMP TABLE vin_meta (
  ...
  spec_variant_name text,
  spec_net_kwh      numeric,
  email_slug        text,
  net_kwh           numeric
);
...
WITH raw AS (
  ...
  WHERE t.type = 'charge' AND t.processed = false
),
with_gap AS (
  SELECT *,
...
WITH raw AS (
  ...
  FROM tessie_raw_imports t
  WHERE t.type = 'drive' AND t.processed = false
),
with_gap AS (
  SELECT *,
    LEAD(started_at) OVER (PARTITION BY vin ORDER BY started_at) - ended_at AS gap_to_next
...
-- -----------------------------------------------------------------------------
-- 7. Processed-Flag setzen
-- -----------------------------------------------------------------------------
UPDATE tessie_raw_imports SET processed = true
WHERE type IN ('charge', 'drive') AND processed = false;
