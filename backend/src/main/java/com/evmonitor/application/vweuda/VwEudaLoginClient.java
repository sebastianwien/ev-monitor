 ...
 * {@link VwEudaAuthException}.
 * <p>
 * Ein Client pro Login-Versuch (eigenes Cookie-Jar), siehe {@link VwEudaLoginClientFactory}.
 */
public class VwEudaLoginClient {
    ...
    public VwEudaLoginClient(VwEudaHttp http, String brandKey) {
        this.http = http;
     ...
     *
     * @throws VwEudaAuthException.InvalidCredentials   E-Mail oder Passwort falsch
     * @throws VwEudaAuthException.InteractionRequired  VW verlangt eine Aktion im Browser
     * @throws VwEudaAuthException.PortalUnavailable    VW-ID oder Portal nicht erreichbar
     */
    public Map<String, String> login(String email, String password) {
        follow(VwEudaHttp.Request.get(PORTAL + "/", baseHeaders(null)));

        Landing identifierPage = follow(VwEudaHttp.Request.get(authorizeUrl(), baseHeaders(null)));
        ...
        Landing passwordPage = follow(VwEudaHttp.Request.postForm(identifierAction,
        ...
        if (!IDP_HOST.equals(hostOf(authenticateUrl))) {
            throw new VwEudaAuthException.InteractionRequired("Passwort-Schritt nicht bei der VW-ID: " + hostOf(authenticateUrl));
        }
        Landing result = follow(VwEudaHttp.Request.postForm(authenticateUrl,
    ...
    private void ensurePortalSession(Landing result) {
        ...
        if ("loginAuthenticate".equals(template)) {
            if (errorCode.isEmpty() || CREDENTIAL_ERROR_CODES.contains(errorCode)) {
                throw new VwEudaAuthException.InvalidCredentials();
            }
            throw new VwEudaAuthException.InteractionRequired(errorCode);
        }
        if (!template.isEmpty()) throw new VwEudaAuthException.InteractionRequired(template);
        ...
        if (url.contains("terms") || url.contains("consent") || url.contains("mfa") || url.contains("onboarding")) {
            throw new VwEudaAuthException.InteractionRequired(URI.create(result.url()).getPath());
        }
        if (result.status() >= 500) throw new VwEudaAuthException.PortalUnavailable("Portal-Login HTTP " + result.status());
        throw new VwEudaAuthException.InteractionRequired("Login endete ohne Portal-Session (HTTP " + result.status() + ")");
    }

    private void failIfNotLoginStep(Landing page, String expectedTemplate) {
        if (page.status() >= 500) throw new VwEudaAuthException.PortalUnavailable("VW-Login HTTP " + page.status());
        String template = templateName(page.html());
        if (!expectedTemplate.equals(template)) {
            throw new VwEudaAuthException.InteractionRequired(template.isEmpty() ? "unerwartete Seite" : template);
        }
    }
    ...
    private VwEudaHttp.Response send(VwEudaHttp.Request request) {
        ...
        VwEudaHttp.Response r = http.send(new VwEudaHttp.Request(request.method(), request.url(), headers, request.formBody()));
        cookies.addFromResponse(request.url(), r);
        return r;
    }

    /** Folgt Redirects manuell (POST wird zu GET), sammelt unterwegs alle Cookies ein. */
    private Landing follow(VwEudaHttp.Request request) {
        VwEudaHttp.Request current = request;
        for (int hop = 0; hop <= MAX_REDIRECTS; hop++) {
            VwEudaHttp.Response r = send(current);
            String location = r.header("Location");
            if (r.status() / 100 == 3 && location != null) {
                current = VwEudaHttp.Request.get(resolve(current.url(), location), baseHeaders(current.url()));
                continue;
            }
            return new Landing(r.status(), current.url(), r.text());
        }
        throw new VwEudaAuthException.PortalUnavailable("Redirect-Schleife beim Login");
    }
    ...
    static String csrfToken(String html) {
        ...
        if (!m.find()) throw new VwEudaAuthException.InteractionRequired("Login-Seite ohne CSRF-Token");
        return m.group(1);
    }

    private JsonNode templateModel(String html) {
        JsonNode model = templateModelOrNull(html);
        if (model == null) throw new VwEudaAuthException.InteractionRequired("Login-Seite ohne templateModel");
        return model;
    }
    ...
    private static String text(JsonNode node, String field) {
        ...
        if (v.isEmpty()) throw new VwEudaAuthException.InteractionRequired("Login-Seite ohne Feld " + field);
        return v;
    }
    ...
    static void requireVwHost(String url) {
        ...
        if (!"https".equalsIgnoreCase(uri.getScheme()) || !ALLOWED_HOSTS.contains(host)) {
            throw new VwEudaAuthException.InteractionRequired("Login verlaesst die VW-Hosts: " + uri.getScheme() + "://" + host);
        }
    }
