 ...
 * einmal bei der VW-ID ein ({@link VwEudaLoginClient}) und uebergibt dem Connectors-Service nur die
 ...
 */
@Service
public class VwEudaConnectService {

    private static final Logger log = LoggerFactory.getLogger(VwEudaConnectService.class);
    ...
    private final VwEudaLoginClientFactory clients;
    private final RestTemplate restTemplate;
    private final String connectorsBaseUrl;
    private final String internalToken;
    private final ObjectMapper json = new ObjectMapper();

    public VwEudaConnectService(CarRepository carRepository,
                              UserRepository userRepository,
                              VwEudaAutoSyncEntitlementService entitlement,
                              RateLimitService rateLimit,
                              VwEudaLoginClientFactory clients,
    ...
    public ConnectionStatus connect(UUID userId, UUID carId, String brand, String email, String password) {
        ...
        if (!rateLimit.tryConsumeVwEudaLogin(userId.toString())) throw new RateLimitedException();
    ...
    private ConnectionStatus handOver(UUID carId, SessionHandover handover) {
        ...
        try {
            ...
            if (status == null) throw new VwEudaAuthException.PortalUnavailable("Leere Antwort vom Connectors-Service");
            return status;
            ...
            throw new VwEudaAuthException.PortalUnavailable("Connectors-Service HTTP " + e.getStatusCode().value());
        } catch (RestClientException e) {
            log.warn("[EUDA] Connectors-Service nicht erreichbar fuer carId={}: {}", carId, e.getMessage());
            throw new VwEudaAuthException.PortalUnavailable("Connectors-Service nicht erreichbar");
        }
    }
