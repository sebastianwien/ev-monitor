...
public class VwEudaJsonParser {
    ...
    public VwEudaJsonParser(ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    /** Fuer kleine, bereits im Speicher liegende Exporte (Tests). */
    public VwEudaParseResult parse(InputStream json) throws IOException {
        byte[] bytes = json.readAllBytes();
        return parse(() -> new ByteArrayInputStream(bytes));
    }

    public VwEudaParseResult parse(InputStreamSource source) throws IOException {
     ...
     */
    public VwEudaParseResult parse(InputStreamSource source, boolean lenient) throws IOException {
        ...
        for (SessionDetector detector : DETECTORS) {
            ...
            return new VwEudaParseResult(header.vin(), detector.detect(index));
        }
        if (lenient) {
            log.debug("EU Data Act: kein Detektor greift ({} Felder) - keine Ladedaten in dieser Datei",
                    header.fieldNames().size());
            return new VwEudaParseResult(header.vin(), List.of());
        }
