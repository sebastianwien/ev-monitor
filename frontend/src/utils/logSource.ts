...
export function sourceInfo(ds?: string | null): LogSourceInfo | null {
  switch (ds) {
    ...
    case 'EU_DATA_ACT_SYNC':    return { label: 'AutoSync',         icon: BoltIcon,          classes: 'bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400 border border-teal-200 dark:border-teal-700' }
    case 'EU_DATA_ACT_IMPORT':  return { label: 'VW Data Act',      icon: ArrowDownTrayIcon, classes: 'bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 border border-purple-200 dark:border-purple-700' }
    default:                    return null
  }
}
