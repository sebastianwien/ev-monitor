    ...
    <div class="flex-1 p-4 flex flex-col gap-4">
      ...
      <div class="flex items-center justify-between gap-2">
        <div class="flex flex-col items-center flex-1">
          <span class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">{{ t('dashboard.efficiency_grid') }}</span>
          <span class="text-base font-bold text-gray-800 dark:text-gray-100 tabular-nums">{{ fmt(efficiencySplit.gridKwh) }}</span>
          <span class="text-xs text-gray-400 dark:text-gray-500">kWh</span>
        </div>

        <div class="flex flex-col items-center px-2">
          <span class="text-xs font-semibold text-amber-500 dark:text-amber-400 tabular-nums">-{{ fmt(lossKwh) }} kWh</span>
          <span class="text-xs text-gray-400 dark:text-gray-500">{{ lossPercent }}% {{ t('dashboard.efficiency_loss') }}</span>
        </div>

        <div class="flex flex-col items-center flex-1">
          <span class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">{{ t('dashboard.efficiency_battery') }}</span>
      ...
      <div>
        ...
        <div class="flex justify-between mt-1">
          <span class="text-xs text-green-600 dark:text-green-400 font-medium">{{ vehiclePercent }}% {{ t('dashboard.efficiency_in_battery') }}</span>
          <span class="text-xs text-amber-500 dark:text-amber-400 font-medium">{{ lossPercent }}% {{ t('dashboard.efficiency_loss') }}</span>
        </div>
      </div>

      <!-- Hinweis bei partieller Abdeckung -->
      <p v-if="isCoveragePartial" class="text-xs text-gray-400 dark:text-gray-500 leading-snug">
