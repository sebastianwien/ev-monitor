...
<template>
  <div class="bg-white dark:bg-gray-800 border-2 border-gray-900 dark:border-white rounded-sm shadow-[4px_4px_0_0_#030712] dark:shadow-[4px_4px_0_0_#ffffff] overflow-hidden flex flex-col">

    <!-- Header: mobile gestackt + einklappbar, sm+ absolut zentriert -->
    <div class="border-b-2 border-gray-300 dark:border-gray-700">
      ...
      <button @click="toggleCollapsed"
        ...
        <div class="flex-1 flex flex-col items-center text-center min-w-0">
          <p class="text-[11px] font-bold uppercase tracking-[0.14em] text-gray-800 dark:text-gray-200 truncate">{{ carDisplayName }} {{ t('dashboard.peer_benchmark_title') }}</p>
          <p class="text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mt-0.5 flex items-center gap-1">
      ...
      <div class="hidden sm:flex relative items-center px-4 py-3">
        <p class="absolute inset-0 flex items-center justify-center text-[11px] font-bold uppercase tracking-[0.14em] text-gray-800 dark:text-gray-200 pointer-events-none">
          {{ carDisplayName }} {{ t('dashboard.peer_benchmark_title') }}
        </p>
        <div class="ml-auto flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 shrink-0 relative">
    ...
    <div class="grid grid-cols-2 divide-x-2 divide-y-2 divide-gray-200 dark:divide-gray-700">

      <!-- Verbrauch (oben links) -->
      <div class="p-4 text-center border-b-0 border-r-0">
        <p class="text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1.5">{{ t('dashboard.peer_consumption') }}</p>
        <div class="flex items-center justify-center gap-1.5 text-sm flex-wrap">
          <span class="font-bold tabular-nums text-gray-900 dark:text-gray-100">{{ formatConsumption(benchmark.userLifetimeConsumptionKwhPer100km) }} kWh</span>
          <span class="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">vs</span>
          <span class="tabular-nums text-gray-500 dark:text-gray-500">Ø {{ formatConsumption(benchmark.peerAvgConsumptionKwhPer100km) }} kWh</span>
          <span v-if="consumptionDelta" :class="['font-bold tabular-nums', consumptionDelta.isGood ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400']">· {{ consumptionDelta.label }}</span>
        </div>
      </div>

      <!-- Reichweite (oben rechts) -->
      <div class="p-4 text-center">
        <p class="text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1.5">{{ t('dashboard.peer_range') }}</p>
        <div class="flex items-center justify-center gap-1.5 text-sm flex-wrap">
          <span class="font-bold tabular-nums text-gray-900 dark:text-gray-100">{{ userRange ?? '–' }} km</span>
          <span class="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">vs</span>
          <span class="tabular-nums text-gray-500 dark:text-gray-500">Ø {{ peerRange ?? '–' }} km</span>
          <span v-if="rangeDelta" :class="['font-bold tabular-nums', rangeDelta.isGood ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400']">· {{ rangeDelta.label }}</span>
        </div>
      </div>

      <!-- Kosten €/kWh (unten links) — nur wenn Daten da -->
      <div v-if="showCost" class="p-4 text-center">
        <p class="text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1.5">
          {{ t('dashboard.peer_cost_label') }}
        </p>
        <div class="flex items-center justify-center gap-1.5 text-sm flex-wrap">
          <span class="font-bold tabular-nums text-gray-900 dark:text-gray-100">{{ formatCost(benchmark.userLifetimeCostPerKwh) }}/kWh</span>
          <span class="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">vs</span>
          <span class="tabular-nums text-gray-500 dark:text-gray-500">Ø {{ formatCost(benchmark.peerAvgCostPerKwh) }}/kWh</span>
          <span v-if="costDelta" :class="['font-bold tabular-nums', costDelta.isGood ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400']">· {{ costDelta.label }}</span>
        </div>
      </div>
      ...
      <div v-if="showCost && userCostPer100km !== null && peerCostPer100km !== null"
        ...
        <p class="text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1.5">{{ t('dashboard.peer_cost_per_distance') }}</p>
        <div class="flex items-center justify-center gap-1.5 text-sm flex-wrap">
          <span class="font-bold tabular-nums text-gray-900 dark:text-gray-100">{{ formatCostPer100km(userCostPer100km) }}/100km</span>
          <span class="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">vs</span>
          <span class="tabular-nums text-gray-500 dark:text-gray-500">Ø {{ formatCostPer100km(peerCostPer100km) }}/100km</span>
          <span v-if="costPer100kmDelta" :class="['font-bold tabular-nums', costPer100kmDelta.isGood ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400']">· {{ costPer100kmDelta.label }}</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="!benchmark.sufficientData"
      class="px-4 py-2.5 border-t-2 border-gray-300 dark:border-gray-700 flex items-center gap-2 mt-auto border-l-2 border-l-amber-500 bg-amber-50 dark:bg-amber-950/30">
