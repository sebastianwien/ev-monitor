...
async function handleConnect() {
  ...
  error.value = null
  try {
    const authStart = await teslaFleetService.getAuthStartUrl()
    if (!authStart.fleetApiConfigured) {
      error.value = 'Tesla Fleet API ist noch nicht konfiguriert (fehlende Developer-Credentials). Bitte wende dich an den Support.'
      fleetApiConfigured.value = false
      return
    }
    if (authStart.authUrl) {
  ...
  <div class="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
    ...
    <template v-if="!status.connected">
      ...
      <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-800 space-y-1">
        ...
        <p class="text-xs text-blue-600 mt-2">Heimladen erscheint über die go-e oder OCPP Integration.</p>
      </div>

      <button
        @click="handleConnect"
        :disabled="isLoading || !fleetApiConfigured"
    ...
    <template v-else>
      <div class="bg-green-50 border border-green-200 rounded-lg p-3">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p v-if="status.lastSyncAt" class="text-xs text-gray-500 mt-0.5">
      ...
      <div v-if="syncResult" class="bg-gray-50 rounded-lg p-3 text-sm">
        ...
        <p v-if="syncResult.logsSkipped > 0" class="text-gray-500 text-xs mt-1">
          {{ syncResult.logsSkipped }} bereits vorhanden, übersprungen
        </p>
      </div>
      ...
      <p class="text-xs text-gray-400 text-center">
        Täglich automatischer Import aktiviert
      </p>
    </template>
  </div>
</template>
